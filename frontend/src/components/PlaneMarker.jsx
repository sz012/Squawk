...
function planeIcon(rot, vector, onGround, selected) {
  //wybrany, w powietrzu, na ziemi
  const color = selected ? '#ffb454' : onGround ? '#4f5d72' : '#f2eadb'
  const glow = selected
    ? 'drop-shadow(0 0 8px rgba(255,180,84,.8))'
    : onGround
      ? 'none'
      : 'drop-shadow(0 0 5px rgba(255,196,120,.45))'
...
export default function PlaneMarker({ flight, selected, onSelect }) {
  ...
  const icon = useMemo(
    () => planeIcon(rot, vector, flight.on_ground, selected),
    [rot, vector, flight.on_ground, selected],
  )
